...
export default function PreviousResults(props) {
  ...
  const formAndAcademicYears = (formsAc) => {
    let academicYearsSorted = [
      ...formsAc
        .reduce((map, obj) => map.set(obj.academicYear, obj), new Map())
        .values(),
    ];
      ...
      <div className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
        <div className="-ml-4 -mt-2 flex flex-wrap items-center justify-between sm:flex-nowrap">
          ...
          <div className="ml-4 mt-2 flex-shrink-0">
            <Link
              href="/portal/exams"
              className="relative inline-flex items-center rounded-md border border-transparent bg-slate-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2"
            >
              Back
            </Link>
          </div>
      ...
      <div className="mx-auto max-w-7xl py-6 px-6 sm:py-2 lg:px-2">
        <div className="mx-auto max-w-3xl divide-y-2 divide-gray-200">
          <dl className="mt-6 space-y-2 divide-y divide-gray-200 pb-8">
            {formAndAcademicYears(getGrades(props.grades).formAndAcademicYears)
              .reverse()
              .map((result, key) => (
                  ...
                  {({ open }) => (
                    <>
                      ...
                      <Disclosure.Panel as="dd" className="mt-2 pr-12">
                        <PreviousResultsReportTemplate
                          formAndAcademicYears={formAndAcademicYears(
                            getGrades(props.grades).formAndAcademicYears
                          )}
                          grades={getGrades(props.grades).grades}
                          forms={forms(getGrades(props.grades))}
                          subjects={allSubjects(getGrades(props.grades))}
                          academicYear={result.academicYear}
                          academicYears={props.academicYears}
                          terms={terms(getGrades(props.grades))}
                        />
                      </Disclosure.Panel>
                    </>
                  )}
