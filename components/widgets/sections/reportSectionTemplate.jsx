...
export default function PreviousResultsReportTemplate(props) {
  ...
  const passRemark = (grades, academicYear, term) => {
    ...
    if (grades == undefined) {
      ...
      grades?.length > 0
        ? grades.map((dd) => {
            if (
              dd?.AcademicYear?.academicYear == academicYear &&
              dd?.AcademicYear?.term == term
            ) {
              contain.push({
  ...
  function mostOccurringGrade(arr){
    return arr.sort((a,b) =>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <tbody className="divide-y divide-gray-200 bg-white">
                  {getTerms(props?.terms?.sort())?.map((term, key) => {
                    if (
                      passRemark(props?.grades, props?.academicYear, term) ==
                      "N/A"
                    ) {
                    } else {
                      return (
                        <tr key={key}>
                          ...
                          {getSubjects(props.subjects.sort()).map(
                            (subj, keys) => {
                              return getGrades(props.grades).map(
                                (grade, key) => {
                                  return grade.term == term &&
                                    grade.year ==
                                      getAcademicYear(props.academicYear) &&
                                    grade.code == subj ? (
                                    <td
                                      key={key}
                                      className="whitespace-nowrap py-4 text-sm font-medium p-1"
                                    >
                                      <span className="text-sky-600">
                                        {subj.toUpperCase()}
                                      </span>
                                      <span
                                        className={`text-${gradeColor(
                                          grade.value
                                        )}-600`}
                                      >
                                        {" "}
